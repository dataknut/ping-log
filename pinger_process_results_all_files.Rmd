...
tpath <- "~/Data/pinger/tmp/" # where to put temporary output - NOT dropbox!
...
```{r errorsByDateHour, fig.width=8}
...
ggplot(allPinger_DT[allPinger_DT$error!="OK"], aes(x = date_hour)) + 
  ...
  facet_grid(label ~ .) +
  scale_x_datetime(date_labels = "%a %d %b", date_breaks = "7 days") +
...
```{r millisecondsByDateTime, fig.cap = "All results for all days (mean & 2 * s.d.)", fig.height=15, fig.width=8}
# make a pretty graph of all data by time with error bars

# create means per hour
ggplot(allPinger_DT, aes(x = date_hour, y = milliseconds)) + 
  # geom_point() + 
  #(stat = "mean", aes(color = host, col = "Response time (ms)")) +
  ggtitle("Mean response times (ms) per hour of each day with +/-2 * s.d.") +
  aes(colour = host) +
...
## Last `r cut + 1` days of data (hubpi only)
Now we will repeat the analysis without aggregation for the last `r cut + 1` days of data that are available and _only_ for the router Pi to be able to inspect the most recent results.

```{r millisecondsByDateTimeRecent, fig.cap = "HubPi results for the last two days (if we have any)", fig.width=8}
# repeat for yesterday & today
# this will break if we have no data from today or yesterday for the hubpi

subset <- allPinger_DT[(r_date > cutDate) & label %like% "BThub"]
...
if(length(subset$label) > 0) {
  ...
  print("Skipping as we have no data for hubpi for the last 4 days.")
}
```
...
```{r millisecondsByDateTimeToday, fig.cap = "All results for today (if we have any)", fig.width=8}
# repeat for yesterday & today
today <- Sys.Date()
...
```{r millisecondsByDateTimeLast2Hours, fig.cap = "All results for the last two hours (if we have any)", fig.width=8}
...
```{r millisecondsByHour, fig.cap = "Mean milliseconds by hour", fig.height=15, fig.width=8}
